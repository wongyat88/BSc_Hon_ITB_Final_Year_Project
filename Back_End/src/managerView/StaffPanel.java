...
public class StaffPanel extends JPanel implements ActionListener{
	...
	public StaffPanel(int locale) {
		...
		String sql = "select * from fyp_staff order by staff_id ASC ";
		emp.runSql(sql);
		jt = new JTable(emp);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == sgo) {
			...
			String sql = "select *  from fyp_staff where "
				+ "(lower(staff_name) like lower('%"+input+"%') or "
				+ "lower(staff_name_zh) like lower('%"+input+"%')) "
				+ "order by staff_id ASC ";
			EmpModel emp = new EmpModel();
			emp.runSql(sql);
			jt.setModel(emp);
		}else if(e.getSource() == add) {
			new StaffDetailDialog(parent,rb.getString("staffDetail_title"),true,localeCurrent,1,"0");
			reFresh();
		}else if(e.getSource() == up) {
			String sid = jt.getValueAt(jt.getSelectedRow(), 0).toString();
			new StaffDetailDialog(parent,rb.getString("staffDetail_title"),true,localeCurrent,2,sid);
			reFresh();
	        ...
	        if (reply == JOptionPane.YES_OPTION) {
	        	String del = "delete from fyp_staff where staff_id=? ";
	        	EmpModel emp = new EmpModel();
	        	String[] paras = {sid};
