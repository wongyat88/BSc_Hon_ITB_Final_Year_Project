...
public class SpecialJPanel extends JPanel implements ActionListener{
	...
	public SpecialJPanel(int locale) {
		...
		String sql = "select si_id, si_name, si_name_zh, si_cat, si_price, si_flag "
				+ "from fyp_sitem order by length(si_id), si_id ASC ";
		emp.runSql(sql);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == b) {
			...
			String sql = "select si_id, si_name, si_name_zh, si_cat, si_price, si_flag "
					+ "from fyp_sitem where"
				+ "(lower(si_name) like lower('%"+input+"%') or "
				+ "lower(si_name_zh) like lower('%"+input+"%')) "
				+ "order by length(si_id), si_id ASC ";
			EmpModel emp = new EmpModel();
			emp.runSql(sql);
			jtable.setModel(emp);
			setTable();
			...
			cat = getCat;
			String sql = "select si_id, si_name, si_name_zh, si_cat, si_price, si_flag "
					+ "from fyp_sitem where si_cat = '"+getCat+"' order by length(si_id), si_id ASC ";
			EmpModel emp = new EmpModel();
			emp.runSql(sql);
			jtable.setModel(emp);
			setTable();
		}else if(e.getSource() == add){
			new SpecialDetailDialog(parent,rb.getString("speicalDetail_title")+" "+rb.getString("fj_add"),true,localeCurrent,1,"0");
			reFresh();
		}else if(e.getSource() == up){
			String sid = "";
			try {
				sid = jtable.getValueAt(jtable.getSelectedRow(), 0).toString();
				new SpecialDetailDialog(parent,rb.getString("speicalDetail_title")+" "+rb.getString("fj_up"),true,localeCurrent,2,sid);
				reFresh();
			}catch (Exception e2) {
			...
			String sid = jtable.getValueAt(jtable.getSelectedRow(), 0).toString();
	        ...
	        if (reply == JOptionPane.YES_OPTION) {
	        	String del = "delete from fyp_sitem where si_id=? ";
	        	EmpModel emp = new EmpModel();
	        	String[] paras = {sid};
