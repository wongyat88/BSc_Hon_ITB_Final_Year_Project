...
public class MaterialComPanel extends JPanel implements ActionListener{
	JFrame parent;
	...
	public MaterialComPanel(int locale) {
		...
		String sql = "select * from fyp_materialcom order by length(c_id),c_id ASC ";
		emp.runSql(sql);
		jt = new JTable(emp);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == b) {
			...
			String sql = "select * from fyp_materialcom where "
			...
			new MaterialComDetailDialog(parent,rb.getString("material_title"),true,localeCurrent,1,"0");
			reFresh();
		}else if(e.getSource() == up) {
			try {
				String mid = jt.getValueAt(jt.getSelectedRow(), 0).toString();
				new MaterialComDetailDialog(parent,rb.getString("material_title"),true,localeCurrent,2,mid);
				reFresh();
			}catch (Exception e2) {}
			...
			try {
		        ...
		        if (reply == JOptionPane.YES_OPTION) {
		        	String del = "delete from fyp_materialcom where c_id=? ";
		        	EmpModel emp = new EmpModel();
		        	String[] paras = {mid};
