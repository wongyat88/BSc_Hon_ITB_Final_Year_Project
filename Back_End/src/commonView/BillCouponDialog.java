...
public class BillCouponDialog extends JDialog implements ActionListener {
	public static int finalDis = 0;
	...
	public static int getFinalDis() {
		return finalDis;
	}

	public BillCouponDialog(Frame owner,String title,boolean modal,int locale,String shopid,String orderid) {
		...
		if(e.getSource() == ok) {
			try {
				...
				try {
					...
					sql = "select c_value,c_mon from fyp_coupon where c_type='"+ctype+"' ";
					emp.runSql(sql);
					String val = emp.getValueAt(0, 0).toString();
